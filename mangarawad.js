...
class Mangarawad extends ComicSource {
    ...
    // genre
    // https://mangarawad.win/search/manga?genre=%E3%82%B3%E3%83%A1%E3%83%87%E3%82%A3
    static labels = ["Ecchi","エロい","コメディ","ロマンス",
        "アクション","スポーツ","ファンタジー","SF","異世界"]
    
        static categories = Mangarawad.labels.map(label => ({
            label,
                /**
                 * @type {PageJumpTarget}
                 */
    ...
    explore = [{
         ...
         */
        load: async(page) => {
            let res = await Network.get(`${Mangarawad.source_url}all-manga/`);
    ...
    category = {
        ...
        parts: [{
            ...
            categories: Mangarawad.categories,
    ...
    categoryComics = {
         ...
         */
        // https://mangarawad.win/search/manga?genre=%E3%82%B3%E3%83%A1%E3%83%87%E3%82%A3&page=2
        load: async(category, param, options, page) => {
            let url = `${Mangarawad.source_url}search/manga?genre=${category}&page=${page}`
            
            if (param === "most-viewed") {
                // https://mangarawad.win/search/manga?status=-1&sort=10&page=2
                url = `${Mangarawad.source_url}search/manga?status=-1&sort=10&page=${page}`
            }
            if (param === "latest-updated") {
                // https://mangarawad.win/?page=2
                url = `${Mangarawad.source_url}?page=${page}`
            }
    ...
    comic = {
        ...
        loadInfo: async(id) => {
            ...
            // 3. 通用解析单元函数
            function parseDoc(doc) {
                let manga = doc.querySelector(".flex.flex-col.md\\:flex-row.mt-4");
                let title = manga.querySelector("h1")?.text?.trim() || "";
                let img = manga.querySelector("img")
                let cover = img.attributes["src"] || img.attributes.src;
                let description = doc.querySelector(".syn-target")?.text?.trim() || "喵喵喵"; //todo
                ...
                for(let item of Array.from(series)) {
                    ...
                    let title = item.querySelector(".font-bold.line-clamp-1")?.text?.trim() || "";
        ...
        loadEp: async(comicId, epId) => {
            let href = epId
            let res = await Network.get(href, Mangarawad.headers);
            ...
            let ul = doc.querySelector(".overflow-y-auto");
            if (!ul) {
                throw `Chapter list not found`;
            }
            let liList = ul.querySelectorAll(".page-chapter");
